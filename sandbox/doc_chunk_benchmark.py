...
from dataset import ToyDocChunkEditDataset
...
def run_doc_chunk_benchmark():
    print("\n=======================================================")
    print("      RUMA DOC-CHUNK BENCHMARK")
    print("=======================================================\n")
    ...
    memory_x, memory_y = dataset.get_update_memory_batch()

    print("[1/6] Training the base model on short factual prompts...")
    ...
    print("[4/6] Running the external-memory doc-chunk update path...")
    memory_model = copy.deepcopy(base_model)
    start = time.perf_counter()
    inserted_records = memory_model.update_memory(
        memory_x,
        target_ids=memory_y,
        sources=[f"doc_chunk_{i}" for i in range(memory_x.shape[0])],
        namespaces=["doc_chunks"] * memory_x.shape[0],
        timestamps=[f"d{i}" for i in range(memory_x.shape[0])],
        causal=True,
    )
    ...
    memory_metrics = {
        ...
        "retrieval": evaluate_memory_retrieval(memory_model, memory_x, inserted_records),
    ...
    print("\n[NOTE] This benchmark tests whether memory written as longer documentation")
    print("       chunks can still update short factual prompts through subsequence matching.")


if __name__ == "__main__":
    run_doc_chunk_benchmark()
